...
class PelangganProvider extends ChangeNotifier {
  ...
  void updatePelanggan(String? uuid, Pelanggan pelangganEdit) {
    ...
    if (index != -1) {
      _pelanggans[index] = _pelanggans[index].copyWith(
        nama: pelangganEdit.nama,
        sandi: pelangganEdit.sandi,
        alamat: pelangganEdit.alamat,
        noTelepon: pelangganEdit.noTelepon,
      );
      notifyListeners();
    }
  }
  ...
  // ini untuk mem-print
  void generateKePdf() async {
    final pdf = pw.Document();

    final fontku = await _loadFont();
    pdf.addPage(
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.ListView.builder(
            ...
            itemBuilder: ((context, index) {
              final n = _pelanggans[index];
              return pw.Text(
                "${index + 1} - $n",
                style: pw.TextStyle(font: fontku), // Specify Roboto font
              );
            }),
          );
        },
    ...
    // ini akan menyimpan file pdfnya {coba nanti cari cara simpan melalui spsesik folder yang
    // ini akan berada di root ketika disimpan}
    Uint8List bytes = await pdf.save();

    // ini akan berada pada root (aplikasinya) bila hanya dir.path

    // -- ini untuk linux
    final dir = await getApplicationDocumentsDirectory();
    final file = File("${dir.path}/docWeb.pdf");
    // -- ini untuk path android
    // final file = File("/storage/emulated/0/Download/docWeb.pdf");

    await file.writeAsBytes(bytes);

    // ini akan langusng membuka file pdf yang kita buat tadi
    await OpenFile.open(file.path);
    print("tersimpan di $file");
  }
